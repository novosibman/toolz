import{cb as Zt,r as m,j as e,F as kt,a as Le,P as ve,cl as nt,ce as Ct,D as Rt,ai as Lt,cm as ea,bh as ta,be as At,by as It,c6 as Se,i as st,U as aa,Q as ra,c as Bt,h as Ae,bF as Nt,bc as Ne,bE as na,E as Ht,b as ct,cn as sa,aj as Dt,ak as Pt,bx as ot,co as se,aC as oa,aK as la,O as St,v as Mt,cp as Fe,bp as ia,az as ca,a_ as da,a$ as ma,aE as ua,bg as fa,cq as ga,cr as pa,bf as ha,bM as xa,av as ba,aw as va,bz as ya,bA as wa,bB as ka,bC as Ca,bD as Na,c0 as Sa,c2 as Ea,c3 as ja,b1 as Et,cs as Ta,X as qe,b2 as Ra,aA as La}from"./common-BJQhp9WS.js";function Aa(a,t,n){const s=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 500" width="800" height="500">
  <defs>
...
</html>`,createdAt:Date.now(),updatedAt:Date.now()};P(S=>({...S,files:[...S.files,C],activeFileId:C.id}))},me=y=>{const b=[];Array.from(y).forEach(C=>{const S=URL.createObjectURL(C),o=C.name.replace(/\s+/g,"-").toLowerCase();b.push({id:`img-${Date.now()}-${Math.random().toString(36).substr(2,5)}`,name:o,path:`src/${o}`,type:"image",content:"",blob:C,blobUrl:S,mimeType:C.type,size:C.size,createdAt:Date.now(),updatedAt:Date.now()})}),b.length>0&&P(C=>({...C,files:[...C.files,...b]}))},c=(y,b)=>{P(C=>({...C,files:C.files.map(S=>S.id===y&&!S.isPrimary&&S.name!=="index.html"&&S.type==="html"?{...S,name:b,path:b,updatedAt:Date.now()}:S)}))},L=y=>{P(b=>{var h;const C=b.files.find(A=>A.id===y);if(!C||C.isPrimary||C.name==="index.html"||C.type==="css")return b;const S=b.files.filter(A=>A.id!==y),o=b.activeFileId===y?((h=S.find(A=>A.type==="html"))==null?void 0:h.id)||"file-index-html":b.activeFileId;return{...b,files:S,activeFileId:o}})},I=y=>{const b=a.files.find(h=>h.id===y);if(!b||b.type!=="html")return;const S=`${b.name.replace(/\.html$/,"")}-copy.html`,o={...b,id:`file-${Date.now()}`,name:S,path:S,isPrimary:!1,createdAt:Date.now(),updatedAt:Date.now()};P(h=>({...h,files:[...h.files,o],activeFileId:o.id}))},q=y=>{const b=y.cssContent||dt(y);P(C=>{const S=C.files.map(o=>o.type==="css"||o.name.endsWith(".css")?{...o,content:b,updatedAt:Date.now()}:o);return{...C,template:y.id,files:S,settings:{...C.settings,themeColor:y.colors.primary}}})},oe=y=>{const b=y.blobUrl||y.externalUrl||y.path,C=document.querySelector('iframe[title="HTML Editor Canvas"]')||document.querySelector('iframe[title="HTML WYSIWYG Canvas"]');if(C!=null&&C.contentWindow)C.contentWindow.postMessage({type:"INSERT_IMAGE",src:b,alt:y.name,name:y.name},"*");else{const S=`
...
`,h=new DOMParser().parseFromString(D.content,"text/html"),A=h.querySelector("main, .content-container")||h.body,O=h.createElement("div");O.innerHTML=S,A.appendChild(O.firstElementChild||O);const W=`<!DOCTYPE html>
${h.documentElement.outerHTML}`;$(W)}},ae=y=>{P(b=>({...b,files:b.files.map(C=>C.type==="css"||C.name.endsWith(".css")?{...C,content:y,updatedAt:Date.now()}:C)}))},$=y=>{P(b=>({...b,files:b.files.map(C=>C.id===D.id?{...C,content:y,updatedAt:Date.now()}:C)}))},ne=y=>{let b="";y==="rich-text"?b='<div class="rich-text" style="margin: 1.5rem 0;"><p>This is a new rich text block. You can highlight text, change colors, bold, and format freely.</p></div>':y==="heading"?b="<h2>New Section Heading</h2>":y==="button"?b='<div style="margin: 1rem 0;"><a href="contact.html" class="btn btn-primary">Button (OK) →</a></div>':y==="card"?b='<div class="card" style="margin: 1.5rem 0;"><h3>Card Title</h3><div class="rich-text"><p>Card content description with customizable styling.</p></div><a href="about.html" class="btn btn-outline btn-sm">Learn More</a></div>':y==="image"?b='<div style="margin: 1.5rem 0;"><img src="src/local.png" alt="Featured Image" class="img-responsive"></div>':y==="divider"&&(b='<hr style="margin: 2rem 0; border: 0; border-top: 1px solid var(--border-color);">');const S=new DOMParser().parseFromString(D.content,"text/html"),o=S.querySelector("main, .content-container")||S.body,h=S.createElement("div");h.innerHTML=b,h.firstElementChild&&o.appendChild(h.firstElementChild);const A=`<!DOCTYPE html>
${S.documentElement.outerHTML}`;$(A)},ge=y=>{const b=y.replace(/^\.\//,"").replace(/^.*\//,""),C=a.files.find(S=>S.type==="html"&&(S.name===b||S.path.endsWith(b)));C&&V(C.id)};return n?e.jsxs("div",{className:"w-full h-screen flex flex-col bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden font-sans",children:[e.jsx(Va,{project:a,onNewProject:()=>v(!0),onOpenProjectManager:()=>j(!0),onExportZip:B,onExportHtml:Z,onUpdateProjectName:Q,isSaving:l}),e.jsxs("div",{className:"flex-1 flex min-h-0 overflow-hidden",children:[e.jsx(Ya,{project:a,activeFileId:D.id,onSelectFile:V,onSelectCssTab:()=>{},onCreatePage:()=>T(!0),onUploadLocalImage:me,onRenameFile:c,onDeleteFile:L,onDuplicateFile:I,onApplyTemplate:q,onInsertImageToActivePage:oe}),e.jsx(Qa,{project:a,activeFile:D,onChangeActiveFileContent:$,onChangeCssContent:ae,selectedNodeId:p,onSelectNodeId:w,onInsertElementToActivePage:ne}),e.jsx(Za,{project:a,activeFile:D,onChangeActiveFileContent:$,onNavigatePage:ge,onInsertElementToActivePage:ne})]}),e.jsx(tr,{isOpen:d,onClose:()=>v(!1),onCreateProject:K}),e.jsx(er,{isOpen:N,onClose:()=>j(!1),currentProjectId:a.id,onSelectProject:H,onNewProject:()=>v(!0)}),e.jsx(ar,{isOpen:R,onClose:()=>T(!1),onCreatePage:ee})]}):e.jsx("div",{className:"w-full h-screen flex items-center justify-center bg-slate-900 text-slate-400 text-xs",children:"Loading Website Builder Studio..."})};export{nr as WebsiteBuilderTool};
